...
```
```bash 生成的证书整理到指定目录
mkdir -p /data/application/openvpn/server/certs
cd /data/application/openvpn/server/certs
cp ../../easy-rsa/3/pki/dh.pem ./				# SSL协商时dh算法需要key
cp ../../easy-rsa/3/pki/ca.crt ./				# ca 根证书
cp ../../easy-rsa/3/pki/issued/server.crt ./			# openvpn服务器证书
cp ../../easy-rsa/3/pki/private/server.key ./			# openvpn 服务器key证书
cp ../../easy-rsa/3/ta.key ./					# tls-auth key
```
...
102 ifconfig-pool-persist /data/logs/openvpn/ipp.txt
141 push "route 172.16.0.0 255.240.0.0"				# 允许LAN路由范围
...
function create_ovpn_cert {
    ...
    echo '<tls-auth>' >> $OVPN_File_DIR/$OUser.ovpn ; grep -v '^#' /data/application/openvpn/server/certs/ta.key >> $OVPN_File_DIR/$OUser.ovpn ;  echo '</tls-auth>' >> $OVPN_File_DIR/$OUser.ovpn
