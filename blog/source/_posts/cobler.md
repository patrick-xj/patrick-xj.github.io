...
6. **添加kickstart文件**  [参考文档](https://access.redhat.com/documentation/en-us/red_hat_enterprise_linux/7/html/installation_guide/chap-kickstart-installations)

    kickstart 文件分为3部分
    ```bash 第1部分,命令部分
    此部分必须在文件顶端: 定义时区、语言、键盘类型、磁盘分区、root密码、selinux、firewall等相关定义
    ```
    ```bash 第2部分,软件安装
    "%packages"开头,"%end"结尾,中间部分写入需要安装的软件和软件包组
    软件包组,以"@"开头;软件不用"@"。每行一个
    # 例: 最小化安装如下定义
    %packages --nobase
    ...
    ```
    ```bash 第3部分,脚本部分
    # 脚本部分,分为预安装脚本和安装后脚本2部分,无先后顺序
    预安装脚本: 是指在系统未安装之前运行的脚本,此部分一般可用于raid划分等
      # "%pre"开头,"%end"结尾 例:
      %pre --interpreter=/usr/bin/python --log=/tmp/ks-pre.log
      ...
      %end
    
    安装后脚本: 是指系统安装完毕之后,脚本执行的部分,此部分可用于初始化等相关设置
      # "%post"开头,"%end"结尾 例:
      %post --interpreter=/usr/bin/bash --log=/tmp/ks-post.log
      wget http://www.example.com/init.sh
      sh init.sh
      %end
    ```
